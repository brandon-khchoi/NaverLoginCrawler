...
public class LZString {
    ...
    private static final char[] _KEY_STR_BASE64 = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".toCharArray();
    private static final char[] _KEY_STR_URISAFE = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+-$".toCharArray();
    private static final Map<char[], Map<Character, Integer>> _BASE_REVERSE_DIC = new HashMap<char[], Map<Character, Integer>>();

    private static char getBaseValue(char[] alphabet, Character character) {
        Map<Character, Integer> map = _BASE_REVERSE_DIC.get(alphabet);
        if (map == null) {
            map = new HashMap<Character, Integer>();
            _BASE_REVERSE_DIC.put(alphabet, map);
    ...
    public static String decompressFromBase64(final String inputStr) {
        ...
        return LZString._decompress(inputStr.length(), 32, new DecompressFunctionWrapper() {
            ...
            public char doFunc(int index) {
                return getBaseValue(_KEY_STR_BASE64, inputStr.charAt(index));
            }
        });
    }
    ...
    public static String decompressFromEncodedURIComponent(String inputStr) {
        ...
        return LZString._decompress(urlEncodedInputStr.length(), 32, new DecompressFunctionWrapper() {
            @Override
            public char doFunc(int index) {
                return getBaseValue(_KEY_STR_URISAFE, urlEncodedInputStr.charAt(index));
            }
        });
    }
    ...
    private static String _compress(String uncompressedStr, int bitsPerChar, CompressFunctionWrapper getCharFromInt) {
        ...
        for (int i = 0; i < uncompressedStr.length(); i++) {
            context_c = String.valueOf(uncompressedStr.charAt(i));
            ...
            if (context_dictionary.containsKey(context_wc)) {
                ...
                if (context_dictionaryToCreate.contains(context_w)) {
                    if (context_w.charAt(0) < 256) {
                        for (int j = 0; j < context_numBits; j++) {
